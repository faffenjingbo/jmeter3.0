...
public class Summariser extends AbstractTestElement
     ...
     * the time.
     */
    private static final int INTERVAL_WINDOW = 0; // in seconds
    ...
    private static String format(String name, SummariserRunningSample summariserRunningSample, String type) {
        ...
        if ("+".equals(type)) {
            ...
            StringBuilder runningData = new StringBuilder();
            runningData.append(JMeterUtils.getProperty("taskSignkey")).append("|")
                    ...
                    .append(summariserRunningSample.getErrorCount()).append("|")
                    .append(summariserRunningSample.getErrorPercentageString()).append("|")
                    .append(summariserRunningSample.getStartTime()).append("|")
                    .append(summariserRunningSample.getEndTime()).append("|")
                    .append(JMeterContextService.getThreadCounts().activeThreads).append("|")
                    .append(JMeterContextService.getThreadCounts().startedThreads).append("|")
                    .append(JMeterContextService.getThreadCounts().finishedThreads);
            log.info("send data to kafka");
            KafkaProducer.getInstance().send(runningData.toString());
        }
        return sb.toString();
    }
    ...
    //            producer.send(new KeyedMessage(KafkaTopic.YPT_KAFKA_TASK_DATA_TOPIC, msg));
    //        } catch (Exception e){
    //            log.error("failed to send kafka msg " + e);
    //        }
    //
    //    }
    //}
